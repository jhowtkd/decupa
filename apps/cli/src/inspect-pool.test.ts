...
import { createVisualPools, sharedVisualPools } from "./app/assembly/visual-pool.ts";
...
function hangingSpawn(onStart?: () => void) {
  return (
    _command: string,
    _args: string[],
...
async function writeInspectFixture(dir: string): Promise<{ indexPath: string; videoPath: string }> {
...
describe("inspect vs visual encode budget", () => {
  ...
  it("extractUnitFrames sem pool usa o orçamento compartilhado do processo", async () => {
    const dest = await mkdtemp(join(tmpdir(), "inspect-shared-"));
    await mkdir(join(dest, "frames"), { recursive: true });
    const shared = sharedVisualPools();
    let encodeCalls = 0;
    const orig = shared.encode.bind(shared);
    shared.encode = ((work, opts) => {
      encodeCalls += 1;
      return orig(work, opts);
    }) as typeof shared.encode;
    try {
      await extractUnitFrames(
        join(dest, "v.mp4"),
        { id: "u004", start: 0, end: 1 },
        join(dest, "frames"),
        {
  ...
  it("abort do inspect mata o ffmpeg e libera a vaga de encode", async () => {
    const dest = await mkdtemp(join(tmpdir(), "inspect-abort-"));
    await mkdir(join(dest, "frames"), { recursive: true });
    const pool = createVisualPools({ ffmpegLimit: 1, networkLimit: 1 });
    const ac = new AbortController();
    let started = 0;
    const pending = extractUnitFrames(
      join(dest, "v.mp4"),
      { id: "u004", start: 0, end: 1 },
      join(dest, "frames"),
      { pool, signal: ac.signal, spawn: hangingSpawn(() => { started += 1; }) },
    );
    await expect.poll(() => started).toBeGreaterThan(0);
    ac.abort();
    await pending.catch(() => undefined);
    let ran = false;
    await pool.encode(async () => { ran = true; });
    expect(ran).toBe(true);
    expect(started).toBe(1);
  });

  it("runTriage abortado mata o ffmpeg default do inspect", async () => {
    const dir = await mkdtemp(join(tmpdir(), "inspect-triage-abort-"));
    const { indexPath, videoPath } = await writeInspectFixture(dir);
    const pool = createVisualPools({ ffmpegLimit: 1, networkLimit: 1 });
    const ac = new AbortController();
    let started = 0;
