...
async function exportedDirValid(dest: string): Promise<boolean> {
  try {
    ...
    if (typeof manifest.handoffMarkdown !== "string" || await exportedFileSha(join(dest, "handoff.md")) !== manifest.handoffMarkdown) return false;
...
export async function exportApproved(project: Project, dir: string): Promise<string> {
  ...
  try {
    ...
    const handoffMarkdown = `# Handoff — revisão ${project.revision}\n\n`+buildHandoff(project).map(n=>`- ${n.sceneId} · frame ${n.startFrame}, ${n.durationFrames} frames · ${n.destination}: ${n.description}`).join("\n");
    const handoffMarkdownSha = createHash("sha256").update(handoffMarkdown).digest("hex");
    ...
    try {
      ...
      matchesExpected =
        ...
        && existing.handoffMarkdown === handoffMarkdownSha
    ...
    try {
      ...
      await writeFile(join(tmp,"handoff.md"),handoffMarkdown);
      const manifest = {
        handoff: handoffSha,
        handoffMarkdown: handoffMarkdownSha,
