...
Provedor de IA: se `DECUPA_COMPANY_API_KEY` (ou `ZAI_API_KEY` / `GEMINI_API_KEY` / `MINIMAX_API_KEY`) já estiver no ambiente desta máquina, o setup grava `~/.decupa/credentials` e o formulário é pulado. A chave do Jev é `TYPESAFE_API_KEY` (ou `DECUPA_COMPANY_TYPESAFE_API_KEY`) e entra no mesmo arquivo. Sem chave de análise, o usuário informa provedor no formulário local na primeira abertura. Nunca solicite a chave no chat.
...
3. Execute node scripts/setup.mjs na raiz do clone e registre cada etapa impressa com duração. Aguarde o preparo obrigatório de Whisper small, VAD, alinhamento PT-BR e MediaPipe. Não deixe os downloads para o primeiro vídeo nem declare sucesso se algum modelo falhar ao carregar. Se falhar no meio, corrija a causa (pré-requisito, rede) e repita o comando — o setup é retomável e preserva estado existente. Se ele recusar por motor existente modificado/em outra revisão, não force: reporte a mensagem exata e peça a decisão. Se falhar por código do produto (ex.: localização de npm/Node), pare a etapa, registre o erro exato com versão do Node e caminho do executável, e reporte como problema para o revisor em vez de contornar.

4. Abra com node scripts/start.mjs --project "<pasta absoluta nova>" (montagem) ou --input "<vídeo autorizado>" (limpeza com ingestão local automática). Se o ambiente tinha chave de empresa, o setup já gravou `~/.decupa/credentials` e o editor abre sem formulário. Senão, conclua o formulário local antes de validar o editor. Salvar valida os campos e grava a chave com proteção local; não verifica saldo, acesso remoto nem suporte visual. Nunca marque esta etapa concluída com chave fictícia. Confira a URL http://127.0.0.1:<porta>, GET / respondendo (e /project na montagem), interface visível, encerramento limpo com Ctrl+C e porta liberada; reabra em seguida e deixe o app funcionando para o usuário após o teste, registrando como ele foi iniciado. Porta ocupada: use --port com outra porta, sem matar processos desconhecidos.
