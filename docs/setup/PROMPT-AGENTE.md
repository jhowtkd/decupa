...
Provedor de IA: obrigatório na primeira abertura. O usuário informa provedor e chave no formulário local do app; nunca solicite a chave no chat.
...
4. Abra com node scripts/start.mjs --project "<pasta absoluta nova>" (montagem) ou --input "<vídeo autorizado>" (limpeza com ingestão local automática). Na primeira abertura, conclua o formulário obrigatório de provedor antes de validar o editor. Salvar valida os campos e grava a chave com proteção local; não verifica saldo, acesso remoto nem suporte visual. Nunca marque esta etapa concluída com chave fictícia. Confira a URL http://127.0.0.1:<porta>, GET / respondendo (e /project na montagem), interface visível, encerramento limpo com Ctrl+C e porta liberada; reabra em seguida e deixe o app funcionando para o usuário após o teste, registrando como ele foi iniciado. Porta ocupada: use --port com outra porta, sem matar processos desconhecidos.
...
7. Provedor é obrigatório e fica em ~/.decupa/credentials (0600 em POSIX; ACL restrita no Windows), reutilizado em novos projetos. Credenciais específicas do projeto continuam tendo precedência. MCP é separado e só é integrado quando solicitado. Nesta referência o transporte MCP usa Content-Length, incompatível com o stdio padrão delimitado por nova linha: só registre MCP operacional após handshake/listagem de ferramentas reais no host; senão, use o terminal do agente e registre MCP bloqueado. Não implemente adaptadores nem correções fora do escopo.
