...
- [x] **Step 1: escrever o teste unitário falhando**
...
- [x] **Step 3: implementar `applyEdit` em `revisions.ts`**
...
- [x] **Step 4: rodar o teste unitário e ver passar**

Run: `pnpm vitest run apps/cli/src/app/assembly/revisions.test.ts`
Expected: PASS

- [x] **Step 5: trocar a rota `/edit` para usar `applyEdit`**
...
- [x] **Step 6: teste de contrato HTTP edit→preview→export**
...
- [x] **Step 7: rodar o fluxo inteiro**
...
- [x] **Step 1: teste de contrato falhando**
...
- [x] **Step 2: rodar e ver falhar** — `pnpm vitest run apps/cli/src/app/assembly/routes.test.ts` → FAIL 404.
...
- [x] **Step 4: rodar e ver passar** — `pnpm vitest run apps/cli/src/app/assembly/routes.test.ts` → PASS.
...
- [x] **Step 1: escrever os testes de paridade (falham: módulo vazio)**
...
- [x] **Step 2: rodar e ver falhar** — `pnpm vitest run apps/cli/src/app/assembly/editor/montage.test.ts` → FAIL (exports indefinidos).

- [x] **Step 3: implementar `montage.js`**
...
- [x] **Step 4: rodar e ver passar** — PASS.
...
- [x] **Step 2: rodar e ver falhar** → FAIL.

- [x] **Step 3: implementar os dois módulos** (state ~15 linhas; api ~25: `fetch` + `res.json().catch(()=>({}))` + contagem de em-voo; portar o espírito de `trackStart/trackEnd` do page.js sem tocar DOM).
...
- [x] **Step 1: escrever page.html/page.css** — grid das 4 regiões com o tema atual (variáveis de cor do `page.css` vigente), dropzone cobrindo `#texto` quando `project.assembly.sources.length === 0`, e `<script type="module" src="/page.js">`. Esqueleto exato:
...
- [x] **Step 5: typecheck + commits granulares**
...
- [x] **Step 1: teste da lógica de menu** (`menuActionsFor`) — casos: mantida, removida, protegida, mista ( throwing: seleção mista mantida+removida não oferece `tirar`; documente no teste), omitida. FAIL primeiro, PASS depois.
- [x] **Step 2: teste do cabeçalho** — `sceneHeaderActions(index, total)` → `[{move:"up",disabled:index===0},...]` puro.
- [x] **Step 3: DOM** — portar `takeWords`/`selectionKey`/`pruneSelection`/`selectedTake` do page.js usando `state`, implementar os listeners de gesto (um `click` handler delegado no `#texto`; o arraste usa `selectionchange`/`pointermove` com limiar de 6px).
- [x] **Step 4: verificação manual** — clicar palavra busca no player; arrastar abre menu; ações do menu alteram o texto e o riscado restaura; preservar sem liberar trava `tirar` (erro do servidor aparece no status do rail, como hoje).
- [x] **Step 5: commit** — `feat: text-center editing gestures with inline action menu`.
...
- [x] **Step 1: testes das puras** (fixture do montage.test.ts): FAIL → PASS.
- [x] **Step 2: DOM + ligação com o playhead** — o player já emite tempo via `timeupdate`; `page.js` encadeia `player.on timeupdate → state.set("playhead", currentTime)`.
- [x] **Step 3: verificação manual** — play acende o bloco da cena atual; clicar/arrastar busca; bloco de apoio não busca áudio diferente (seek é na montagem toda).
- [x] **Step 4: commit** — `feat: sequence strip as navigation compass`.
...
- [x] **Step 1: teste de peaks** — `waveform.test.ts`: executor fake que grava PCM determinístico (silêncio + senoide) → `buildPeaks` resolve com N buckets min/max; executor que falha (code 1) → `null`. FAIL → PASS.
- [x] **Step 2: implementar `waveform.ts`** — ffmpeg no proxy (mono 8kHz s16le → min/max por bucket), JSON `<sha>.peaks.json` ao lado do proxy; toda exceção vira `null`.
- [x] **Step 3: chamar na etapa media** — em `preparation.ts`, logo após o proxy da fonte existir: `await buildPeaks(...).catch(() => null)` sem entrar no caminho crítico (falha não marca a fonte com erro).
- [x] **Step 4: teste da rota** — `routes.test.ts`: `GET /project/waveform/<id>` → 200 JSON com peaks; fonte sem peaks → 204.
- [x] **Step 5: teste de `retainedSegments`** — puro, em `montage.test.ts`: take com remoção no meio vira 2 segmentos com `montageStart` correto.
- [x] **Step 6: canvas na faixa** — `sequencia.js` busca peaks por fonte (cache em Map), desenha segmentos por `retainedSegments` na proporção de `timelineBlocks`; apoio fora do waveform.
- [x] **Step 7: verificação manual** — faixa com waveform no projeto de prova; sem peaks a faixa segue usável (só blocos).
- [x] **Step 8: commit** — `feat: best-effort source waveform peaks composed on the strip`.
...
- [x] **Steps**: teste `watchedState` (6 casos) FAIL→PASS; fiação do player; chip + cadeado; teste manual: aprovar sem assistir até o fim é impossível (botão desabilitado e o POST não parte); commit `feat: real watched-through tracking gates approval and delivery`.
...
- [x] **Step 1: teste de preservação do caminho NL (spec, Testes)** — em `revisions.test.ts`: `applyProposal` com proposta que altera uma cena **fora** de `changedSceneIds` lança `"proposta reescreve cena ... fora do escopo"`; e uma proposta válida que muda só `s2` deixa `s1` byte-a-byte igual (`expect(next.scenes[0]).toEqual(before.scenes[0])`). Se já existir caso equivalente no arquivo, confira e siga — o objetivo é o teste existir travando a preservação.

- [x] **Step 2: implementar confirmação inline + rótulos** — botão "Preparar montagem" abre confirmação inline (não `confirm()` nativo): "Vai analisar N arquivo(s) — custo estimado do modelo visual + envio das mídias ao provedor. Continuar?" com [Preparar agora] [Cancelar]. Se `project.permissions.visual === true` já concedido, mostra "já autorizado (persistente)" em vez da pergunta (honestidade do spec: permissão é monotônica).

- [x] **Step 3: pedido NL com rótulo de custo** — botão mostra "(modelo pago)" quando `!permissions.model` e "já autorizado" quando concedido; envio segue passando `modelOptIn/visualOptIn` atuais nas flags de `/project/prepare|adjust` (o contrato do servidor não muda). Os checkboxes `modelOptIn`/`visualOptIn` do header morrem com o header.

- [x] **Step 4: verificação** — `pnpm vitest run tests/assembly-flow.test.ts apps/cli/src/app/assembly/revisions.test.ts` (402 e preservação verdes); manual: prepare sem permissão mostra o lote; segunda vez mostra "já autorizado".

- [x] **Step 5: commit** — `feat: paid consent at batch dispatch with honest granted state`.
...
- [x] **Step 1: teste falhando** — em `assembly-flow.test.ts`: dispara `/project/analyze` (executor fake lento via promise controlada), faz `/project/edit` (remove palavra) durante, solta o executor, espera o poll terminar; hoje o save do analyze/prepare falha 409; esperado: 200 com `analyses` presentes **e** a edição preservada (`scenes[0].takes[0].removed.length === 1`, `revision` avançou 2x).

Run: `pnpm vitest run tests/assembly-flow.test.ts` → FAIL antes, PASS depois.

- [x] **Step 2: implementar o rebase no save do percurso** (preparation.ts; o ponto exato é o `saveProject` pós-etapas — aplique sobre `current` as partes produzidas: `analyses` merge via `mergeAnalyses` já importado em routes.ts/store.ts).

- [x] **Step 3: commit** — `fix: preparation saves rebase over concurrent edits instead of 409`.
