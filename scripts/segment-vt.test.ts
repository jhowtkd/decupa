...
it("lê o codec da primeira linha do ffprobe (grupo de streams repete o codec)", async () => {
  expect(await python("import json\nfrom segment_vt import first_codec\nprint(json.dumps([first_codec('hevc\\n\\nhevc\\n'), first_codec('h264\\n'), first_codec('')]))"))
    .toEqual(["hevc", "h264", ""]);
});

async function cut(opts: { codec: string; failVt?: boolean; platform?: string; env?: NodeJS.ProcessEnv }): Promise<{
  installed: boolean; vtCalls: number; x264Calls: number; result: string | null; mixed: boolean;
}> {
  return await python(`
...
${FAKE_ENGINE}
import segment_vt
installed = segment_vt.install(engine, platform=${JSON.stringify(opts.platform ?? "darwin")}, probe=lambda p: ${JSON.stringify(opts.codec)})
...
print(json.dumps({
  ...
  "result": result,
  "mixed": segment_vt.state["mixed"],
}))
`, opts.env) as never;
}

it("fonte HEVC no macOS: segmento sai em VideoToolbox", async () => {
  expect(await cut({ codec: "hevc" })).toEqual({ installed: true, vtCalls: 1, x264Calls: 0, result: null, mixed: false });
});

it("fonte H.264 continua em libx264 (VideoToolbox não compensa ali)", async () => {
  expect(await cut({ codec: "h264" })).toEqual({ installed: true, vtCalls: 0, x264Calls: 1, result: null, mixed: false });
});

it("VideoToolbox que falha refaz o segmento com o comando original do motor", async () => {
  expect(await cut({ codec: "hevc", failVt: true })).toEqual({ installed: true, vtCalls: 1, x264Calls: 1, result: null, mixed: false });
});

it("fora do macOS ou com DECUPA_RENDER_SOFTWARE=1 não instala", async () => {
  expect((await cut({ codec: "hevc", platform: "win32" })).installed).toBe(false);
  expect((await cut({ codec: "hevc", env: { DECUPA_RENDER_SOFTWARE: "1" } })).installed).toBe(false);
});
...
FAIL_VT = False
${FAKE_ENGINE}
import segment_vt, concat_copy
segment_vt.install(engine, platform="darwin", probe=lambda p: "hevc")
...
print(json.dumps({"mixed": segment_vt.state["mixed"], "concat": copied}))
`);
